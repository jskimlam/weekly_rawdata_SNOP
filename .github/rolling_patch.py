...
helper = r'''
/* ---------- 1P 가격표 자동 롤링 규칙 ---------- */
let MATRIX_REFERENCE=null;
const MATRIX_BASELINE_YEAR=2026;
const MATRIX_BASELINE_MONTH=8;

function matrixCaptureReference(){
  if(MATRIX_REFERENCE)return MATRIX_REFERENCE;
  const out={};
  const table=document.querySelector('#p1 .price-matrix');
  if(!table)return out;
  [...table.querySelectorAll('tbody tr[data-item]')].forEach(tr=>{
    const item=tr.dataset.item,c=tr.cells,v=i=>num(c[i]?.textContent);
    out[item]={
      months:{'2026-08':v(8),'2026-07':v(9),'2026-06':v(10),'2026-02':v(11),'2025-08':v(15)},
      yearAvg:{'2026':v(12),'2025':v(13)},
      december:{'2025':v(14)},
      high:{'2025':v(16)},
      low:{'2025':v(17)}
    };
  });
  MATRIX_REFERENCE=out;
  return out;
}
...
function matrixYearAverage(item,year,week){
  const ref=matrixCaptureReference()[item]?.yearAvg?.[String(year)];
  const d=parseDate(week);
  if(year===MATRIX_BASELINE_YEAR&&ref!==null&&ref!==undefined&&Number.isFinite(Number(ref))){
    if(!d||d.getFullYear()<MATRIX_BASELINE_YEAR||d.getMonth()+1<=MATRIX_BASELINE_MONTH)return Number(ref);
    let total=Number(ref)*MATRIX_BASELINE_MONTH,count=MATRIX_BASELINE_MONTH;
    for(let m=MATRIX_BASELINE_MONTH+1;m<=d.getMonth()+1;m++){
      const v=matrixObservedMonthAverage(item,year,m,1);
      if(v!==null){total+=v;count++;}
    }
    return count?total/count:Number(ref);
  }
  if(ref!==null&&ref!==undefined&&Number.isFinite(Number(ref)))return Number(ref);
  const vals=matrixSeries(item).filter(x=>{const z=parseDate(x.week);return z&&z.getFullYear()===year&&(!d||year!==d.getFullYear()||x.week<=week);}).map(x=>Number(x.value));
  return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;
}
function matrixDecember(item,year){
  const ref=matrixCaptureReference()[item]?.december?.[String(year)];
  if(ref!==null&&ref!==undefined&&Number.isFinite(Number(ref)))return Number(ref);
  return matrixMonthAverage(item,year,12);
}
function matrixYearExtreme(item,year,kind){
  const ref=matrixCaptureReference()[item]?.[kind]?.[String(year)];
  if(ref!==null&&ref!==undefined&&Number.isFinite(Number(ref)))return Number(ref);
  const series=matrixSeries(item).filter(x=>{const d=parseDate(x.week);return d&&d.getFullYear()===year;});
  const vals=series.map(x=>Number(x.value));
  const months=new Set(series.map(x=>parseDate(x.week).getMonth()+1));
  if(vals.length<8||months.size<6)return null;
...
if '/* ---------- 1P 가격표 자동 롤링 규칙 ---------- */' not in s:
    anchor = 'function updatePriceMatrix(week){'
    if anchor not in s:
        raise SystemExit('updatePriceMatrix anchor not found')
    s = s.replace(anchor, helper + '\n' + anchor, 1)

new_func = r'''function updatePriceMatrix(week){
  ...
  for(const item of PRICE_ROW_ITEMS){
    ...
    // c[18] Historical High/Low는 장기 기준값이므로 기존 값을 유지
  }
}
'''

pat = re.compile(r'function updatePriceMatrix\(week\)\{.*?\n\}\nfunction setMatrix', re.S)
s2, n = pat.subn(new_func + '\nfunction setMatrix', s, count=1)
if n != 1:
    raise SystemExit(f'updatePriceMatrix replacement count={n}')
